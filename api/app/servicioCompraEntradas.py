...
#     def validar_compra(self, forma_pago: str, entradas: List[Entrada], usuario_id: int, fecha_visita: str):
#         forma_pago = self.validar_forma_pago(forma_pago)
#         fecha_visita = self.validar_fecha_visita(fecha_visita)
#         usuario = self.validar_usuario_registrado(usuario_id)
#         self.validar_cantidad_entradas(len(entradas))
#         for entrada in entradas:
#             self.validar_entrada(entrada)
#         # Si todas las validaciones pasan, se procede a crear la compra
#         compra = self.crear_compra_entradas_y_pago(entradas, usuario, forma_pago)
#         estado_envio_de_mail = "PENDIENTE"
#         if forma_pago == "efectivo":
#             estado_envio_de_mail = self.enviar_mail_confirmacion_efectivo(compra)
#         return compra, len(compra.entradas), compra.fecha, estado_envio_de_mail



#     def procesar_pago_tarjeta(self, id_compra: int):
...
#             raise PagoError("El pago de la compra ya se realizo por Mercado Pago")
#         # Simulamos el procesamiento del pago con tarjeta
#         exito_de_pago = random.random() > 0.01
#         if not exito_de_pago:
...
#         compra.pago.codigo_pago = random.randint(100000, 999999)  # Simulamos un código de pago generado
#         compra = self.repositorio.actualizar_pago_compra(compra)
#         estado_envio_de_mail = self.enviar_mail_confirmacion_tarjeta(compra)
#         return compra, len(compra.entradas), compra.fecha, estado_envio_de_mail



#     def validar_fecha_visita(self, fecha_visita):
#         fecha_visita = date.fromisoformat(fecha_visita)
...
#     def validar_cantidad_entradas(self, cantidad):      
#         if cantidad <= 0:
...
#     def validar_usuario_registrado(self, usuario_id):
...
#     def validar_entrada(self, entrada: Entrada):
#         if entrada.edad_visitante < 0:
#             raise ValidacionError("La edad del visitante no puede ser negativa")
#         if entrada.tipo_pase not in ["VIP", "Regular"]:
#             raise ValidacionError("El tipo de pase no es válido")
#         if entrada.precio < 0:
#             raise ValidacionError("El precio de la entrada no puede ser negativo")
#         return True


#     def crear_compra_entradas_y_pago(self, entradasValidas: List[Entrada], usuario: Usuario, forma_pago: str):
#         compra = Compra(entradasValidas,usuario)
#         if forma_pago == "efectivo":
...
#         self.repositorio.crear_compra(compra)
#         return compra

#     def enviar_mail_confirmacion_efectivo(self, compra: Compra):
#         print(f"Enviando mail de confirmación de pago en efectivo para la compra {compra.id_compra}")
#         return "ENVIADO"

#     def enviar_mail_confirmacion_tarjeta(self, compra: Compra):
